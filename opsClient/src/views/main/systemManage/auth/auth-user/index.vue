...
export default defineComponent({
  setup() {
    ...
    const Myresult = ref();
    const changeLevel = () => {
      let params = { type: "getAuth", username: selectValue.value };
    ...
    const changeAuth = () => {
      let params = { type: "changeAuth", username: selectValue.value, authList: transferValue.value };
      getMenuApi(params).then((res: { data: any; }) => {
        let myRes = res.data;
        if (myRes.myState == "success") {
    ...
    function getUserList() {
      let params = { type: "userList" };
      userInfoApi(params).then((res2: { data: { myState: string; myDesc: any; }; }) => {
        if (res2.data.myState == "success") {
          Myresult.value = res2.data.myDesc;
          selectOptions.value = [];
          cities.value = [];
          Myresult.value.forEach(function (row: any) {
            selectOptions.value.push({ value: row[0], label: row[1]+"("+row[0]+")" });
            cities.value.push(row[0]);
          })
